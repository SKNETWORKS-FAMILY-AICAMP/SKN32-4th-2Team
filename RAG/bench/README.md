...
- 아래 명령은 프로젝트 루트(`D:\Dev_Tools\SKN32-4th-2Team`)에서 실행한다. 다른 위치에서 실행한다면 `RAG\...` 경로를 그 위치에 맞게 바꾼다.
...
- 대상은 `RAG/.env`가 새 운영 DB `rag_chatbot_v4`를 가리키고, [RAG 초기화 절차](../../SETUP.md#2-첫-실행-db-마이그레이션과-pdf-초기화)를 마친 현재 PDF·FAISS 코퍼스여야 한다. 이전 DB의 `document`·PDF·`vector_store`는 벤치마크 입력으로 재사용하지 않는다.
...
FAISS 인덱스를 그대로 비교하지 말고 동일 코퍼스를 `bootstrap_documents.py --apply --mode overwrite`로 다시 인덱싱한 뒤 새 manifest를 생성해야 합니다. 재색인 전후 결과는 같은 기준선으로 합치지 않습니다.
