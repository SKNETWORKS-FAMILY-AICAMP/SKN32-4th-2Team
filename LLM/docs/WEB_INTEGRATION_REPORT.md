...
> **기록 문서(2026-08-03)입니다.** 당시 FastAPI/SQLite·RAG mock 및 별도 테스트 폴더에서
> 수행한 확인을 남긴 것이며, 현재 Django 전환의 실행 안내나 MySQL 설정값으로 사용하면 안 됩니다.
> 현재 공통 대상 DB는 `rag_chatbot_v4`이고, 레거시 DB에서는 사용자·채팅·`chat_source` 스냅샷만
> 이관합니다. 현재 절차는 [WEB README](../../web/README.md)와 [전체 실행 가이드](../../SETUP.md)를 따릅니다.

> 작성: Member D (LLM) · 08/03 오전 1차 확인 · **08/03 저녁 2차 확인 추가**
> **당시 결론: 당시 브랜치의 WEB↔LLM 호출 흐름은 확인했습니다.** 아래 기록의 SQLite·RAG mock
> 범위를 벗어난 현재 Django/MySQL·새 RAG 코퍼스의 동작을 보증하는 문서는 아닙니다.
